...
# ---------------------------------------------------------------------------
# **مهلةٌ على قفل dpkg، ومهلةٌ على الخطوة كلّها.**
#
# نشرٌ يوم 2026-09-03 علّق هنا أكثر من عشرين دقيقة: `unattended-upgrades`
# كان يحمل القفل، و`apt-get` بلا مهلةٍ ينتظر أبداً. والنشر يعمل من طرفيةٍ
# على الماك بلا مدخل — فلا شيء يقول ما الذي يُنتظَر، ويبدو الخادم معلّقاً
# والسبب حزمةٌ تُحدَّث في الخلفية.
#
# ⇒ القفل بمهلة، والخطوة بمهلة، والفشل **يُسمّى**: انتظارُ قفلٍ ليس عطلاً
# في الشبكة ولا في المفتاح، ورسالةٌ واحدة تفرّق بينها توفّر ساعة بحث.
# ---------------------------------------------------------------------------
APT=(apt-get -o DPkg::Lock::Timeout=120 -o Dpkg::Use-Pty=0)
if ! timeout 300 "${APT[@]}" update -qq; then
  die "تعذّر تحديث قوائم الحزم (مهلة ٣٠٠ ثانية — قد يكون قفل dpkg مشغولاً بتحديثٍ تلقائي)."
fi
if ! timeout 600 "${APT[@]}" install -y -qq git python3 python3-venv python3-pip curl ufw jq >/dev/null; then
  die "تعذّر تثبيت الحزم الأساسية (مهلة ٦٠٠ ثانية — راجعي: fuser /var/lib/dpkg/lock-frontend)."
fi
