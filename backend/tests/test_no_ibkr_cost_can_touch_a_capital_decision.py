...
def test_the_honest_gold_run_actually_reached_the_capital_model(honest):
    ...
    **حارسٌ ثالث — وقد كُتب لأن الفحص التالي كان فارغاً بدونه.**
    ...
    # **درجةٌ على سلّم الوسيط.** كان الشرط «الكمية الدنيا بالضبط»، وهو
    # صحيحٌ حين لم يكن ثمّة سلّم. والآن يُختار الأقرب إلى الهدف من مضاعفات
    # الدرجة — والدليل نفسه يبقى: كميةٌ على سلّم كابيتال، لا كسرٌ محلولٌ
    # عكسياً من ميزانية أسهم.
    q = honest.risk_decision.quantity
    assert q >= D("0.01") and (q / D("0.01")) % 1 == 0, (
        f"الكمية {q} ليست درجةً على سلّم الوسيط — أي أن مسار الأسهم هو الذي سعّر."
    )
