...
def main() -> int:
    ...
    if not positions:
        print(f"\n{OK}✅ لا مراكز مفتوحة على الحساب التجريبي **الآن**.{END}")
        # ولا تُفسَّر القراءة: هذه الأداة تقرأ الحاضر ولا تعرف تاريخه. ومركزٌ
        # فُتح ثم أُغلق يعطي القراءة نفسها التي يعطيها مركزٌ لم يُفتح قطّ —
        # وقولُ «لم يترك مركزاً» عن الحالتين ادّعاءٌ لا تسنده القراءة.
        print(f"{DIM}   وهذه قراءةُ حاضرٍ لا تاريخ: لا تقول أفُتح مركزٌ وأُغلق،{END}")
        print(f"{DIM}   أم لم يُفتح أصلاً. سجلّ التدقيق وحده يقول ذلك.{END}\n")
        return 0
