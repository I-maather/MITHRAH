...
class SystemState:
    ...
    #: والمطابقة. تُحدَّث كل دورة، **وحتى أثناء الإيقاف المحلي**: الإيقاف
    #: يمنع فتح المراكز، لا رؤيتها.
    portfolio: object = None
    #: نتيجةُ آخر مزامنةٍ للدفتر الدائم (`LedgerSync`) — ما فُتح وما أُغلق
    #: وما تغيّر وما لم يُنسَب. `None` قبل أوّل مزامنةٍ ناجحة.
    ledger_sync: object = None
    #: عطلُ كتابة الدفتر إن وقع. الكتابة لا تُسقط الدورة: القراءة نجحت
    #: والمراقبة قائمة، فيُسجَّل العطل ولا تتوقّف الحراسة.
    ledger_error_ar: str = ""
    #: خطّةُ إدارة المراكز لهذه الدورة (`ManagementPlan`) — تُبنى ولو كان
    #: النظام موقوفاً: الإيقاف يمنع الفتح لا الإدارة.
    management_plan: object = None
    management_error_ar: str = ""
    #: تقريرُ الإقلاع (`StartupReport`). **لا دخولَ جديد قبل أن يجتاز.**
    #: الفحص كان مكتوباً منذ 0.2 ولم يُستدعَ من أي مكان — وحدةٌ سليمة غير
    #: موصولة، وهو نفس عيب طبقة الجوال قبل 0.5.6.
    startup: object = None
