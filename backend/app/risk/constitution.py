...
class ModeSpec:
    ...
    #: **سقفُ حرارة المحفظة** — بالدولار عند رأس المال المرجعي، ويُقاس تناسبياً.
    #:
    ...
    #: يمنع ذلك قبل وقوعه.
    #:
    #: القيمةُ الدولارية هي المقصودة حرفياً هنا، والنسبةُ تُنتج كسراً دورياً
    #: يفسد المقارنة (10 ÷ 300 لا تُمثَّل بدقّة) — فتُعطى بالدولار كما
    #: `target_risk_usd`.
    #:
...
MODE_SPECS: dict[RiskMode, ModeSpec] = {
    RiskMode.VALIDATION: ModeSpec(
        ...
        # ---------------------------------------------------------------
        # ═══════════════════════════════════════════════════════════
        # **رفعُ المخاطرة — قرارُ المالكة ١١ سبتمبر ٢٠٢٦، في التجريبي وحده.**
        #
        # «ابغا ارفع مستوى المخاطرة عشان يقدر يكسب اكثر… High Risk = High
        # Return. بس اول شي بجرب بالحساب التجريبي.»
        #
        # وشرطُها المُعلَن: **بعد** صفقاتٍ تجريبية (٦ مغلقة عند التنفيذ)،
        ...
        # حدّ اليوم 15 ⇒ إعدادٌ يُرفَض عند البناء ولا يُقلع الخادم به.
        # فاختارت المالكة تقليل المراكز على رفع حدّ اليوم إلى 18: أسوأُ يومٍ
        # ممكن 12 دولاراً (٤٪) بدل 18 (٦٪).
        #
        # ومع الدلو المشترك صارت الأدواتُ الأربع ثلاثةَ دلاء أصلاً، فالمركز
        # الثالث كان نادراً على أي حال.
        #
        # **وما لم يتغيّر:** الأوضاعُ الحقيقية لم تُمَسّ. وهذه زيادةٌ في
        # حجم النتيجة لا في احتمالها: تُوسّع الربحَ والخسارةَ معاً، ولا
...
class RiskLimits:
    ...
    def allocated_margin_per_position(self) -> Decimal:
        """
        **سقف الهامش لمركزٍ واحد، من رأس المال المخصَّص لا من رصيد الحساب.**

        الحساب التجريبي رصيده نحو ٩١ ألف دولار، ورأس المال المخصَّص
        للاستراتيجية **٣٠٠**. وفحص الهامش كان يقارن بالمتاح عند الوسيط —
        أي أن نظاماً يتداول ٣٠٠ يستطيع أن يحجز آلافاً لأن الرصيد يسمح.

        وذلك يُبطل معنى التخصيص: تجربةٌ ناجحة على ٩١ ألفاً **لا تنتقل** إلى
        ٣٠٠، لأن قيد الكمية الدنيا غير مُلزم هناك ومُلزمٌ في كل صفقة هنا.

        ⇒ السقف = رأس المال المخصَّص ÷ أقصى عدد مراكز متزامنة. فمجموع
        الهوامش المحجوزة لا يتجاوز المخصَّص مهما بلغ رصيد الحساب.

        وهو **حجزٌ لا خسارة** — والخسارة يحكمها سقف المخاطرة، وهذان قيدان
        مستقلّان لا يغني أحدهما عن الآخر.
        """
        positions = max(1, int(self.max_open_positions))
        return self.baseline_equity / Decimal(positions)
    ...
    def for_mode(
        ...
        return RiskLimits(
            ...
            max_portfolio_risk=_scaled(base, spec.max_portfolio_risk_usd),
        )
