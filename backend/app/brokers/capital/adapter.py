...
class CapitalComAdapter(BrokerAdapter):
    ...
    def list_recent_transactions(
        ...
        **الصيغة مقصودة ومقيسة — وقياسُها الأوّل كان ناقصاً.** كان هنا
        مكتوباً أنّ `?from=...&to=...` «تُعيد errorCode بلا بيانات». وهي
        تعمل؛ والذي كان يُردّ هو **الصيغة** لا المسار. قيس على الديمو في
        ٢٠٢٦-٠٩-١٩:

            from=2026-09-14T00:00:00   → 200 ببيانات
            from=…T00:00:00Z           → 400
            from=2026-09-14            → 400
            lastPeriod=604800          → 400 error.invalid.lastPeriod

        وكلفةُ الخلاصةِ المكتوبة بلا تجربتها: أربعةُ أيامٍ من القفل، لأنّ
        سقفَ `lastPeriod` يومٌ واحد فلا يبلغ دليلَ إغلاقٍ أقدم منه.
        و`?lastPeriod=` تُجيب داخل اليوم بهذا الشكل:
    ...
    def place_order(self, intent: OrderIntent) -> BrokerOrder:
        ...
        **الاعتراف بالنجاح لا يأتي من استجابة الإرسال.** `POST /positions`
        ...
        return BrokerOrder(
            ...
            updated_at_utc=at,
            # `position_ids_of` كانت تُحسَب لتتحقّق من الوقف ثمّ تُرمى.
            # وهي الجسرُ الوحيد بين الأمر والمركز، فتُحفَظ الآن.
            position_deal_ids=tuple(
                dict.fromkeys(
                    tuple(self.position_ids_of(confirmation)) + (str(position.deal_id),)
                )
            ),
        )
