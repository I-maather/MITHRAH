...
export function drawCandles(svg, bars, opts = {}) {
  ...
  // **الاتجاهُ يُثبَّت.** الصفحةُ RTL، و`text-anchor="start"` داخل سياقٍ
  // معكوس يعني الحافةَ اليمنى — فكانت وسومُ المستويات تمتدّ خارج اللوحة
  // وتُقصّ. الرسمُ إطارٌ إحداثيٌّ لا نصّ، فاتجاهُه ثابت.
  ...
  // **المقياسُ من الشموع وحدها.** ضمُّ الهدف والوقف إليه يمطّ المحور حتى
  // تنكمش الشموعُ في شريطٍ رفيعٍ وسط فراغ — وهو ما حدث فعلاً: هدفٌ خارج
  // نطاق اليوم سحب المقياس فصار الرسمُ عن المستويات لا عن السعر.
  // والمستوى الواقعُ خارج الإطار لا يُحذف: يُعلَّم على الحافة بسهم.
  const shown = bars.slice(-60);
  let lo = Infinity, hi = -Infinity;
  for (const b of shown) { if (b.l < lo) lo = b.l; if (b.h > hi) hi = b.h; }
  if (live != null && Number.isFinite(live)) { lo = Math.min(lo, live); hi = Math.max(hi, live); }
  ...
  for (const lv of levels) {
    const tone = lv.tone === "neg" ? NEG : lv.tone === "pos" ? POS : LINE;
    const outside = lv.price < lo || lv.price > hi;
    const yy = outside ? (lv.price > hi ? PAD_T + 4 : H - PAD_B - 4) : y(lv.price);

    if (!outside) {
      svg.appendChild(el("line", {
        x1: PAD_L, x2: W - PAD_R, y1: yy, y2: yy,
        stroke: tone, "stroke-width": 1, "stroke-dasharray": "3 5", opacity: 0.45,
      }));
    }
    if (lv.label) {
      const t = el("text", {
        x: PAD_L + 3, y: yy - 3.5, fill: outside ? tone : INK3,
        "font-size": "8", "text-anchor": "start", opacity: outside ? 0.8 : 0.85,
      });
      // خارج الإطار: سهمٌ يقول الاتجاه بدل خطٍّ يكذب بموضعه.
      t.textContent = outside
        ? `${lv.label} ${lv.price > hi ? "↑" : "↓"} ${lv.price.toFixed(digits)}`
        : lv.label;
      svg.appendChild(t);
    }
  }
