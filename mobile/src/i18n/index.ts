...
//: أيام الأسبوع وأشهر السنة بالعربية — لا تُشتقّ من `toLocaleDateString`:
//: مخرجُها يتغيّر بلغة الجهاز، فتظهر «Monday» لمن جهازه بالإنجليزية.
...
/**
 * «الاثنين ٣١ أغسطس» — عنوان الشاشة الرئيسية.
 *
 * كان العنوان كلمة «الرئيسية»، وهي مكتوبة أصلاً في شريط التبويبات وفي هيدر
 * التنقّل — ثلاث مرّات في شاشة واحدة. والتاريخ يقول شيئاً لا يقوله غيره:
 * **أيّ يومٍ هذا الحكم**، وهو ما تحتاجه المالكة حين تفتح التطبيق بعد يومين.
 */
export function formatToday(at: Date = new Date()): string {
  return `${WEEKDAYS_AR[at.getDay()]} ${at.getDate()} ${MONTHS_AR[at.getMonth()]}`;
}
