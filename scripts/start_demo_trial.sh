...
#       bash scripts/start_demo_trial.sh "أوافق على تشغيل التداول التجريبي"
#
# ولرفع الإيقاف المحلي في الأمر نفسه (فيتداول وحده عند فتح السوق):
#
#     MATHRAH_BASELINE=300 DEMO_TRIAL_RESOLUTION=HOUR_4 \
#       MATHRAH_RESUME="ارفع الإيقاف المحلي" \
#       bash scripts/start_demo_trial.sh "أوافق على تشغيل التداول التجريبي"
#
# ## ما يفعله
#
...
#: رفع الإيقاف المحلي من هنا — **بالعبارة نفسها التي يطلبها الخادم**.
#:
#: `locally_paused` يبدأ موقوفاً في كل إقلاع، وهذا مقصود: نظامٌ يتداول
#: بمجرّد أن يُقلع ليس نظاماً يُوثَق به. والرفع كان يحتاج فتح التطبيق —
#: خطوةً ثانية قد تفوت لحظة فتح السوق.
#:
#: فالحارس **لم يُخفَّف، نُقل**: العبارة نفسها تُكتب هنا بيد المالكة،
#: ويرفضها الخادم إن لم تُطابق حرفاً. ولا قيمة افتراضية لها، ولا تُشتقّ
#: من عَلَمٍ صامت.
RESUME_PHRASE_REQUIRED="ارفع الإيقاف المحلي"
RESUME="${MATHRAH_RESUME:-}"
...
if printf '%s' "$OUT" | grep -q '"local_trading_paused": *true\|"local_trading_paused":true'; then
  if [ "$RESUME" = "$RESUME_PHRASE_REQUIRED" ]; then
    step "٤ · رفع الإيقاف المحلي — بعبارتك"
    RESUME_OUT="$("${SSH[@]}" "curl -s --max-time 10 -X POST \
      http://127.0.0.1:8000/api/trading/resume \
    ...
    if [ -z "$PAUSED_SEEN" ]; then
      ...
      red "⛔ لم يُرفع. الخادم ما زال يقول موقوف — لا يُدَّعى غير ذلك."
      dim  "   ارفعيه من التطبيق: النظام ← الطوارئ ← «استئناف التداول»."
    fi
    echo
  else
  printf '\033[33m⚠️  التداول المحلي ما زال موقوفاً — ولن تُفتح صفقة حتى ترفعيه.\033[0m\n'
  dim  "   من التطبيق: النظام ← الطوارئ ← «استئناف التداول»"
  dim  "   أو في هذا الأمر: MATHRAH_RESUME=\"$RESUME_PHRASE_REQUIRED\""
  echo
  fi
else
